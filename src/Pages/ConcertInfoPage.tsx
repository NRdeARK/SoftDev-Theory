...
const ConcertInfoPage = () => {
  ...
  const getUserDataFromCookie = () => {
    if(cookies){
      ...
      return userDataString ? userDataString : null;
    }
    return null
  };

  const userData = getUserDataFromCookie(); // user data from cookies
  ...
  const selectedConcert = concertData.find(
    (concert) => concert.id === concertId
  );

  // กรองให้เหลือแต่ role hiring
  const hiringRecipients = recipients.filter(
    (recipient) => recipient.role === "hiring"
  );
      ...
      <div className="container" id="info">
        <div className="column" id="concertInfo">
          <div className="container" id="concertHeader">
            ...
            <h2 id="concertDate">
              {selectedConcert?.Start} - {selectedConcert?.End}
            </h2>
          </div>
          <img id="concertPic" src={selectedConcert?.PhotoUrl} alt=""></img>
          ...
          <div>
            <p id="concertText">
              วั้นพร้อมมั้ย ทไว้พร้อมล้าว! TWICE 5TH WORLD TOUR ‘READY TO BE’ IN
              BANGKOK วันที่ 23 - 24 ก.ย. อิมแพ็ค อารีน่า เมืองทองธานี
            </p>
            <p id="concertText">
              รอบการแสดงวันที่ 24 กันยายน 2566 จำหน่ายบัตรทั่วไป ( 23 มิถุนายน
              2566 เวลา 10.00 น. เป็นต้นไป) ที่{" "}
              <a href="https://www.SafeTicket.com">www.SafeTicket.com</a>{" "}
              เท่านั้น และเปิดจำหน่ายครบทุกช่องทางวันที่ 24 มิถุนายน ตั้งแต่เวลา
              10.00 น. เป็นต้นไป <br></br>• ลูกค้า 1 ท่าน ซื้อบัตรได้ไม่เกิน 4
              ใบ <br></br>• การซื้อบัตรทางเว็บไซต์ในวันที่ 23 มิ.ย. 66
              โปรดกดรอรับคิวซื้อบัตรได้ที่หน้าเว็บไซต์เวลา 9.00 น.
              และจำหน่ายบัตรเวลา 10.00 น. เป็นต้นไป
            </p>
          </div>
        </div>
